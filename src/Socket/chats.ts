...
export const makeChatsSocket = (config: SocketConfig) => {
	...
	const setPinnedState = async (state: boolean, jid: string) => {
		await chatModify({ pin: state }, jid)
	}

	const setLabels = async (labels: string[], chats: string[]) => {
		for (const chat of chats) {
			for (const label of labels) {
				await chatModify({ addLabel: { label } }, chat)
			}
		}
	}
